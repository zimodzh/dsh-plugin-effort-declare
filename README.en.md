...
> **Note:** This skill is a community-maintained project. It is not affiliated with DeepSeek, and is not officially endorsed.
...
- Three draft presets (save is still explicit):
  - **DeepSeek compatible** — empty `off` plus identity `low` / `high` / `max`; route `thinkingFormat: deepseek` and `supportsDeveloperRole: false`.
  - **OpenAI compatible** — `minimal` / `low` / `medium` / `high`; omits `thinkingFormat` (default openai); does not force `supportsDeveloperRole: false`.
  - **On/off only** — `off` + `high` and `supportsReasoningEffort: false`. The UI warns that extra selector levels are identical on the wire.
...
| Tests | In-repo Vitest: presets, save semantics, clear, validation without throw, list filtering |
...
This plugin was built against DeepSeek Harness **0.1.0-rc.8**. Install it on DSH **0.1.0-rc.8 or newer**. Older versions are untested and not guaranteed to work. You also need a bootable profile.

**From a local checkout:**

```bash
dsh plugin --profile <profile> add /path/to/dsh-plugin-effort-declare
dsh --profile <profile> --dump-config   # expect a dsh-plugin-effort-declare layer
```

**From GitHub** (replace `OWNER` with the actual upstream):

```bash
dsh plugin --profile <profile> add github:OWNER/dsh-plugin-effort-declare
```

If pnpm refuses the git dependency `prepare` script (`tsdown`), add the package to that profile’s `pnpm-workspace.yaml` `allowBuilds` and `add` again. This repo commits `lib/`, so a built checkout or a `pnpm pack` tarball avoids build authorization.

After DSH starts, open **Settings → Reasoning efforts**. Add a hand-declared `llm-pi-ai` provider on the Models page first; this page only lists editable routes.
...
3. The advanced panel (collapsed by default) exposes `thinkingFormat`, `supportsDeveloperRole`, and `supportsReasoningEffort`.
4. Save. Client-side validation failures (Off only, empty `reasoningEfforts`) are shown in the panel and never crash apply; official schema rejections surface the Host error the same way.
